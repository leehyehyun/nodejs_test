...
exports.update_process = function(request,response){
    ...
    request.on('end', function(){
        var post = qs.parse(body); // form으로 받아온 정보 가져오기 (id/name/profile)
...
exports.delete_process = function(request,response){
    ...
    request.on('end', function(){
        ...
        `, [post.id], function(error, result){ // 지우려는 author가 쓴 글 먼저 삭제
            if(error){
                throw error;
            }
            db.query(`DELETE FROM author WHERE id=?
            `, [post.id], function(error2, result){ // 지우려는 author 삭제
            if(error2){
                throw error2;
            }
            response.writeHead(302, {Location: `/author`});
